...
function isVerifiedMicRecord(raw){
  ...
  if(!(status==='verified'||status==='verified-database'||status==='verified_database'||status==='spec_record'))return false;

  const name=String(raw.name||'').trim();
  if(!name)return false;

  const rangeFields=[raw.mic,raw.hot,raw.tail,raw.ceil];
  const hasRange=rangeFields.every(v=>v!==undefined&&v!==null&&String(v).trim()!=='');
  ...
  if(!hasRange){
    ...
    return specFields.some(v=>v!==undefined&&v!==null&&String(v).trim()!=='');
  }

  const mic=Number(raw.mic),hot=Number(raw.hot),tail=Number(raw.tail),ceil=Number(raw.ceil);
  if(!Number.isFinite(mic)||!Number.isFinite(hot)||!Number.isFinite(tail)||!Number.isFinite(ceil))return false;
